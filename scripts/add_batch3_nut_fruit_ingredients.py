#!/usr/bin/env python3
"""
Script to add nut/fruit ingredients for batch 3 recipes:
1. Sunflower Seeds
2. Apple
3. Raw Almonds
"""
...
def main():
    """Main function to add all nut/fruit ingredients."""
    db = get_db()
    
    try:
        print("=" * 60)
        print("Adding Nut/Fruit Ingredients for Batch 3 Recipes")
        print("=" * 60)
        
        # Step 1: Add Sunflower Seeds
        print("\n--- Step 1: Adding Sunflower Seeds ---")
        create_ingredient_with_conversions(
            name='Sunflower Seeds',
            type_name='Nuts & Seeds',
            shopping_unit_name='package',
            conversion_rules=[
                {'from': 'tablespoon', 'factor': 1/30.27},  # 1/30.27 = 0.0330 packages/tbsp (16g/tbsp)
        ...
        create_ingredient_with_conversions(
            ...
            conversion_rules=[
                ...
                {'from': 'cup', 'factor': 125/180},  # 125/180 = 0.694 whole/cup (diced, ½-inch)
                {'from': 'gram', 'factor': 1/180},  # 1/180 = 0.00556 whole/g (medium)
            ],
            size_rules=[
                {'size': 'small', 'unit': 'gram', 'value': 150},
                {'size': 'medium', 'unit': 'gram', 'value': 180},
                {'size': 'large', 'unit': 'gram', 'value': 220},
            ],
            db=db
        )
        
        # Step 3: Add Raw Almonds
        print("\n--- Step 3: Adding Raw Almonds ---")
        create_ingredient_with_conversions(
            name='Raw Almonds',
            type_name='Nuts & Seeds',
            shopping_unit_name='package',
            conversion_rules=[
                {'from': 'cup', 'factor': 100/454},  # 100/454 = 0.220 packages/cup (sliced/slivered)
                {'from': 'tablespoon', 'factor': 1/30.27},  # 1/30.27 = 0.0330 packages/tbsp (6g/tbsp sliced)
                {'from': 'gram', 'factor': 1/454},  # 1/454 packages/g
                {'from': 'ounce', 'factor': 1/16},  # 1/16 = 0.0625 packages/oz
            ],
            db=db
        ...
        print("\n" + "=" * 60)
        print("✓ All nut/fruit ingredients added successfully!")
        print("=" * 60)
